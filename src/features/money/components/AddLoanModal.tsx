    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center animate-[mon-fade-in_150ms_ease-out]" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between px-5 py-4" style={{ borderBottom: '1px solid var(--mon-border)' }}>
          <h3 className="text-[16px] font-bold" style={{ color: 'var(--mon-text-1)' }}>{t.addLoan}</h3>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <div className="flex p-1 rounded-[var(--mon-radius-lg)]" style={{ background: 'var(--mon-surface-2)', border: '1px solid var(--mon-border)' }}>
            {(['given', 'taken'] as const).map((d) => (
              <button key={d} type="button" onClick={() => setDirection(d)}
                className={`flex-1 py-2 rounded-md text-[13px] font-semibold transition-all ${direction === d ? 'text-white shadow-sm' : 'text-[var(--mon-text-3)]'}`}
                style={direction === d ? { background: d === 'given' ? 'var(--mon-income)' : 'var(--mon-expense)' } : {}}
              >
                {(t as any)[d]}
              </button>
            ))}
          ...
          <div>
            ...
            <input type="text" value={personName} onChange={(e) => setPersonName(e.target.value)} placeholder="e.g. John Doe" autoFocus
          ...
          <button type="submit" disabled={!personName || !amount}
