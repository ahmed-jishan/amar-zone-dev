...
interface Props {
  ...
  onReconcile: (walletId: string, balance: number) => void
...
export default function WalletToolsModal({ wallets, selectedWalletId, onClose, onTransfer, onReconcile, onAddWallet, currencySymbol }: Props) {
  ...
  const handleSubmit = (event: React.FormEvent) => {
    ...
    if (mode === 'transfer') {
      ...
      onReconcile(fromWalletId, numericBalance)
    }
    onClose()
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center animate-[mon-fade-in_150ms_ease-out]" onClick={onClose}>
      ...
      >
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-[16px] font-bold" style={{ color: 'var(--mon-text-1)' }}>Wallet Tools</h3>
        ...
        <div className="space-y-3">
          ...
          {mode === 'transfer' ? (
            ...
            <AmountInput label="New balance" value={balance} onChange={setBalance} currencySymbol={currencySymbol} />
          )}
