...
export default function TaskDetailsModal({ task, onClose }: Props) {
  ...
  return (
    <BottomSheet
      open={true}
      onClose={onClose}
      snapPoints={[0.45, 0.7, 0.92]}
      title="Task Details"
    >
      {/* Segmented Tab Control */}
      <div className="flex items-center gap-1 p-1 rounded-[var(--az-radius-xl)] bg-[var(--az-surface-2)] border border-[var(--az-border)] mb-5 overflow-x-auto">
        {tabs.map((tab) => (
          <button
            key={tab.key}
            onClick={() => {
            ...
            className={`
              relative px-3 py-2 rounded-[var(--az-radius-lg)] text-[12px] font-semibold transition-all duration-200 whitespace-nowrap flex items-center gap-1.5
              ${activeTab === tab.key
      ...
      {/* Footer Actions */}
      <div className="flex items-center justify-between gap-3 mt-6 pt-4 border-t border-[var(--az-border)]">
        <div className="flex items-center gap-2">
          <motion.button
            whileTap={{ scale: 0.95 }}
